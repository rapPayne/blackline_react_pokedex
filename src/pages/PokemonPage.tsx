...
export const PokemonPage = () => {
	...
	const showSprites = (sprites: object) =>
		Object.entries(sprites).map((pi, i) => {
	...
	return (
		<>
			{fetching ? <h1>LOADING, PLEASE WAIT</h1> : null}
			{/* <pre>{JSON.stringify(pokemon.sprites, null, 2)}</pre> */}
			<h1>Pokemon Page for {name}</h1>
			<section className="fullWidth">
				<section className="left">{sprites && showSprites(sprites)}</section>
				<section className="infoBox">
					...
					<ul>
						{types?.map((type, index) => (
							<li
								key={index}
								style={{ color: getColorForType(type.type?.name) }}
							>
								{type.type?.name}
							</li>
						))}
